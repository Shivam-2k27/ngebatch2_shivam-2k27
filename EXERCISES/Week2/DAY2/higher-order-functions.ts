...
export function countFromEurope(developers: Developer[]) {
  ...
  if (count == 0 || developers == null) return 0;
  else return count;
}
...
export function getGreetings(developers: Developer[]) {
  return developers.map(
    (items) =>
      `Hi ${items.firstName}, what do you like the most about ${items.language}?`
  );
}
...
export function getAverageAge(developers: Developer[]) {
  if (developers.length == 0) return -1;
    ...
    console.log(
      Math.floor(
        developers.reduce((age, value) => age + value.age, 0) /
          developers.length
      )
    );
}
...
// For the list above, it would return { Javascript: 1, Python: 2 } [js ,python,python]
...
export function getLanguageCounts(developers: Developer[]) {
  ...
  developers.map((items) => {
    ...
    if (countLanguages[lang])
      //we check if the language is already a property of Language object if it is we increase the count
      countLanguages[lang]++;
    else countLanguages[lang] = 1; //else we initialize it
  });
...
export function getOldest(developers: Developer[]) {
  ...
  if (developers.length > 0) {
    ...
    developers.map((items) => {
      if (items.age == oldstDev) newArray = [...newArray, `${items.firstName}`]; //appending the firstName as string to the newArray using spread operator
    });
    return newArray;
...
export function isGlobalGroup(developers: Developer[]) {
  const zonerArray = [
    { ctry: "Africa", count: 1 },
    { ctry: "Americas", count: 1 },
    { ctry: "Asia", count: 1 },
    { ctry: "Europe", count: 1 },
    { ctry: "Oceania", count: 1 },
  ];
  ...
  if (zonerArray.reduce((a, b) => a + b.count, 0) == 0) return true;
  else return false;
}
