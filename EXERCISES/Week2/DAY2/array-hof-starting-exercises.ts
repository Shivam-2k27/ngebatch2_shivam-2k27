...
const shoutOut = academitePets.map((items)=>`Hello ${items}`)
console.log('Shout out is', shoutOut)

// Let's use filter to remove Daisy as she's been naughty today
const filtered = academitePets.filter((items)=>items!='Daisy')
...
const above12Months = academitePetsWithAges.filter((item)=>item.age>12)
...
const filteredAndShouted = academitePetsWithAges.filter((age)=>age.age<12).map((items)=>items)
