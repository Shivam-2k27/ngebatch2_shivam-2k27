...
const getPersonalHelloMessage = () => {
  ...
  fetch('http://localhost:8080/hello/:shivam/').then(response =>response.text()).then(
    data=>{const content = document.createElement('p')
    content.innerText = `Hello ${data} welcome `
    helloDiv!.appendChild(content)}
  )
}
